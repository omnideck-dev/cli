...
func (m AppModel) renderMetaPanel(inst *InstanceState, w, h int) string {
	cfg := inst.Info.Config
	innerW := w - 2
...
func (m AppModel) renderResourcePanel(inst *InstanceState, w, h int) string {
	barW := w - 22
	if barW < 4 {
		barW = 4
	}
...
func (m AppModel) renderLogsPreview(inst *InstanceState, w, h int) string {
